...
def add_environment_toTable(ds, tab, edate, envvar_take=[],tabvar_skip=[], rainvar=False, env_tformat="%Y-%m-%d %H:%M:%S", env_hour=12, env_min=0):
    """
    ...
    for tlat, tlon, date, mask, tir in zip(dic['tminlat'], dic['tminlon'], dic['date'], dic['cloudMask'], dic['tir']):
        ...
        if rainvar:
            if len(envvar_take)==0:
                ...
                rain_var_name = envvar_take[0]
            tabdate = pd.to_datetime(date, format=tab_tformat)  # rainfall sampling same time as TIR

            pos = envdates.hour == tabdate.hour
            rain = ds.isel(time=pos).where(mask).squeeze()
        ...
        if len(envvar_take) > 0:
            if not cft:
                tabdate = pd.to_datetime(edate, format=tab_tformat).replace(hour=env_hour, minute=0)  # hour of environment sampling
            else:
                tabdate = edate.replace(hour=env_hour, minute=env_min)
            pos = envdates == tabdate
            #print(ds,pos,envdates,tabdate)
            ...
            for vt in envvar_take:
                if vt in dic.keys():
                    dic[vt].append(single.values)
                else:
                    dic[vt] = [single.values]
    return dic